 ...
 */
let userInputAge: String? = "-33"

func checkAge(userInputAge: String?) {
    if let userAgeString = userInputAge, let userAge = Int(userAgeString) {
        if userAge >= 18 {
            print("Welcome!")
        } else {
...
func getItemCost(itemName: String) -> Double? {
    if let stockCount = stock[itemName] {
...
if let cost = getItemCost(itemName: "Chips") {
    print("The cost of Chips is $\(cost)")
} else {
    print("Chips is out of stock or not found.")
}
