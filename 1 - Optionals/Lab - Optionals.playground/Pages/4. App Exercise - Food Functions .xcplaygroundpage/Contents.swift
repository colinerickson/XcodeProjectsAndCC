...
func foodFunction(mealType: String) -> Meal? {
...
foodFunction(mealType: "Breakfast")
/*:
...
var mealLog: [String: Meal] = ["Pancakes and Eggs": Meal(food: ["Pancakes", "Eggs", "Toast"], calories: 500)]

func saveMealLogToDevice() {
    
    UserDefaults.standard.set(mealLog, forKey: "mealLog")
    UserDefaults.standard.synchronize() 
}

func isMealSavedToDevice(mealName: String) -> Meal? {
    // Simulate retrieving from UserDefaults (in a real app, this would use UserDefaults)
    if let savedMealLog = UserDefaults.standard.dictionary(forKey: "mealLog") as? [String: Meal],
       let meal = savedMealLog[mealName] {
        return meal
    }
    return nil
}

// Save mealLog to simulated UserDefaults
saveMealLogToDevice()

// Call the function to check if a specific meal is saved
if let savedMeal = isMealSavedToDevice(mealName: "Pancakes and Eggs") {
    print("Found saved meal: \(savedMeal)")
} else {
    print("Meal not found or not saved.")
}
